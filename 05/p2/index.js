...
const processInput = () => {
    ...
    const input = fs.readFileSync('05/input.txt').toString().trim()
    ...
    while (true) {
        ...
        dataSet.push(createMap(lines))
    }
}

const fillSeedRanges = (tokens )  => {
...
const createRangeObj = (start , end)  => {

    return { "start": start, "end": end }
}

const createMapObj = (sourceStart , sourceEnd, destinyStart, destinyEnd, delta)  => {
...
const search = ()  => {
    ...
    for (const map of dataSet) { ranges = searchThis(ranges, map) }
    
    return ranges
}

const searchThis = (oldRanges , map) =>  {
...
const fillRangeGaps = (oldRange , doneRanges) => {
